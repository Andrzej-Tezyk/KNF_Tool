...
def process_pdf(prompt: str, pdf: Path, model: Any, output_size: int) -> dict:
    ...
    try:
        ...
        if len(text) > 10:  # random small number
            response = model.generate_content(f"{prompt} (Please limit the response to approximately {output_size} words) {text}") # , generation_config={"max_output_tokens": max_tokens}

        else:
            file_to_send = genai.upload_file(pdf)
            print(f"PDF uploaded successfully. File metadata: {file_to_send}\n")
            response = model.generate_content([prompt + f"(Please limit the response to approximately {output_size} words)", file_to_send])
