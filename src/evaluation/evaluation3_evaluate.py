...
df = pd.read_excel(
    "https://raw.githubusercontent.com/Andrzej-Tezyk/KNF_Tool/main/src/Q_A_total_scored.xlsx"
)
# for `hallucination_evaluator` the input df needs to have columns 'output', 'input', 'context'
# for `qa_evaluator` the input df needs to have columns 'output', 'input', 'reference'
df["context"] = df["A_real"]
df.rename(
    columns={"Q": "input", "A_tool": "output", "A_real": "reference"}, inplace=True
)
assert all(
...
for i in range(0, len(df), batch_size):
    ...
    if i + batch_size < len(df):
        print(
            f"Processed {i+batch_size} rows, sleeping {pause_seconds}s to respect quota..."
        )
...
pct_label_by_source = (
    results_df.groupby("Q_source")["label_bert"].mean().mul(100).to_dict()
)

# --- 3) % factual ---
summary["pct_factual_overall"] = (
    results_df["hallucination_eval"].eq("factual").mean()
) * 100
pct_factual_by_source = (
    results_df.groupby("Q_source")["hallucination_eval"]
    .apply(lambda x: (x.eq("factual").mean() * 100))
    .to_dict()
)

# --- 4) % correct ---
summary["pct_correct_overall"] = (results_df["qa_eval"].eq("correct").mean()) * 100
pct_correct_by_source = (
    results_df.groupby("Q_source")["qa_eval"]
    .apply(lambda x: (x.eq("correct").mean() * 100))
    .to_dict()
)
...
with pd.ExcelWriter(
    "Q_A_total_evaluated.xlsx", engine="openpyxl", mode="a", if_sheet_exists="replace"
) as writer:
